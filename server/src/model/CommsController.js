...
const populateReminisce = async (req, res) => {
  ...
  try {
    ...
    for (let comm of keep) {
      ...
      const event = await db.meeting.findOne({where: {CommunicationId: comm.dataValues.id}});
      if (!event) return;
...
module.exports = { addCommunication, getCommunication, getMeetings, postMeeting, populateReminisce }