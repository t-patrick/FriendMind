...
function Friends({ navigation, route }: FriendsProps) {
  ...
  const handleButton = (button: string) => {
    if (
      (button === "last" && buttonOneSelected) ||
      (button === "longest" && buttonTwoSelected)
    ) {
      return;
    } else {
  ...
  return (
    <>
      <ScrollView style={styles.top}>
        ...
        <View style={styles.sortButtons}>
          <Button
            ...
            color={buttonOneSelected ? "#1685EC" : "lightgrey"}
            onPress={() => handleButton("last")}
          >
            <Text>Last Seen</Text>
          </Button>
          <Button
            mode="contained"
            style={styles.sortButton}
            color={buttonTwoSelected ? "#1685EC" : "lightgrey"}
            onPress={() => handleButton("longest")}
          >
      ...
      <FAB style={styles.fab} icon="plus" onPress={addFriend} />
    </>
  );
}
