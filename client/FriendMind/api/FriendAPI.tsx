...
const baseUrl = "https://popular-dragonfly-8.loca.lt/";

export const getFriends = async (userId: number) => {
  return fetch(`${baseUrl}/friends?id=${userId}`).then((friends) =>
    friends.json()
  );
};
...
export const getEvents = async (
  ...
  return fetch(`${baseUrl}/meetings?id=${friendId}`).then((events) =>
    events.json()
  );
};
...
export const addFriend = async (
  userId: number,
  friend: FriendForAdd,
  preferences: Array<CommPreference>
) => {
...
export const getFriend = async (friendId: number) => {
  return fetch(`${baseUrl}/friend?friendId=${friendId}`).then((friends) =>
    friends.json()
  );
};
